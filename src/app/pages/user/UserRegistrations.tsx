...
import { mockRegistrations, mockActivities } from '../../data/mockData';
import { ClipboardList, Calendar, X } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export function UserRegistrations() {
  const { user } = useAuth();
  const myRegistrations = mockRegistrations.filter((r) => r.userId === '2');

  const handleCancel = (id: string) => {
    if (confirm('Batalkan pendaftaran ini?')) {
      const updated = mockRegistrations.filter((r) => r.id !== id);
      console.log('Pendaftaran dibatalkan:', id, '(demo - data di memori)');
    }
  };
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className={`px-3 py-1 text-xs rounded-full ${
                              ...
                              {registration.status === 'approved' ? 'Disetujui' :
                               registration.status === 'pending' ? 'Menunggu Persetujuan' : 'Ditolak'}
                            </span>
                          </div>
                          ...
                          <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-600">
                            <div className="flex items-center gap-2">
                              <Calendar size={16} />
                              <span>Kegiatan: {new Date(activity?.date || '').toLocaleDateString('id-ID')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <ClipboardList size={16} />
                              <span>Terdaftar: {new Date(registration.registeredDate).toLocaleDateString('id-ID')}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span>📍</span>
